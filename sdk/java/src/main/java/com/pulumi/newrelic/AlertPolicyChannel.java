 ...
 * 
 * &gt; **A note on the availability of this resource in Japan (JP).** Both `newrelic.AlertPolicyChannel` and its companion `newrelic.AlertChannel` are backed by the legacy REST v2 Alert Channels API, which was globally deprecated in 2024 - the same reason both resources have been marked deprecated for a while, as noted in the warning above. Because REST v2 is not provisioned on the New Relic Japan region, neither resource can be used there; a `pulumi preview` against a provider configured with `region = &#34;JP&#34;` fails early with an actionable error message.&lt;br&gt;&lt;br&gt;Moving to the NerdGraph-based `newrelic.Workflow` paired with `newrelic.NotificationDestination` and `newrelic.NotificationChannel` is not a JP-only concern - it is highly essential in every region, so that your alerting keeps working when the REST v2 Alert Channels API is formally sunsetted. See the getting started guide for a walkthrough of wiring alert conditions, alert policies, notification destinations, notification channels, and workflows together in Terraform.
 * 
 * ## Example Usage
 * 

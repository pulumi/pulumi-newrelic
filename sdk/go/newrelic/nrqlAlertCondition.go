...
// - `dataAccountId` - (Optional) **BETA PREVIEW: the `dataAccountId` field is in limited release and only enabled for preview on a per-account basis.** The account ID to use for the alert condition's query as specified in the the `query` field. If `dataAccountId` is not specified, then the condition's query will be evaluated against the `accountId`. Note that the `accountId` must have read privileges for the `dataAccountId` or else the condition will be invalid.

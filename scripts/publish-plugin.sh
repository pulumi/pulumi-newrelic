...
PLUGIN_PACKAGE_NAME="pulumi-resource-newrelic-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
go build \
   -ldflags "-X github.com/pulumi/pulumi-newrelic/pkg/version.Version=${VERSION}" \
   -o "${WORK_PATH}/pulumi-resource-newrelic${BIN_SUFFIX}" \
   "${ROOT}/cmd/pulumi-resource-newrelic"
